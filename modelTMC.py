...
class TMC(nn.Module):
    ...
    def DS_Combin(self, alpha):
        ...
        for v in range(len(alpha)-1):
            if v==0:
                alpha_a,u_a, u0, u1 = DS_Combin_two(alpha[0], alpha[1])
            else:
                alpha_a,u_a, u0, u1 = DS_Combin_two(alpha_a, alpha[v+1])
        return alpha_a,u_a,u0, u1
    ...
    def forward(self, X, y, global_step):
        ...
        alpha_a, u_a, u0, u1 = self.DS_Combin(alpha)
        evidence_a = alpha_a - 1
        loss += ce_loss(y, alpha_a, self.classes, global_step, self.lambda_epochs)
        loss = torch.mean(loss)
        return evidence, evidence_a, loss, u_a, u0, u1
