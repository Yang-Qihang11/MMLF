...
def drawRotatedBox(img, x, y, w, l, yaw, color):
    ...
    # cv2.line(img, (corners_int[0, 0].astype(np.int32), corners_int[0, 1].astype(np.int32)), (corners_int[3, 0].astype(np.int32), corners_int[3, 1].astype(np.int32)), (255, 255, 0), 2)




def drawRotatedBox2(img, x, y, w, l, yaw, color, u_a):
        bev_corners = get_corners(x, y, w, l, yaw)
        corners_int = bev_corners.reshape(-1, 1, 2).astype(int)
        cv2.polylines(img, [corners_int], True, color, 2)
        ...
        corners_int = bev_corners.reshape(-1, 2)
        cv2.line(img, (corners_int[0, 0].astype(np.int32), corners_int[0, 1].astype(np.int32)),
                 (corners_int[3, 0].astype(np.int32), corners_int[3, 1].astype(np.int32)), (255, 255, 0), 2)

        # 在图像上添加目标分数
        text = f"uncertainty: {u_a:.5f}"
        # reversed_text = text[::-1]
        cv2.putText(img, text, (corners_int[0, 0].astype(np.int32), corners_int[0, 1].astype(np.int32) - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

def drawRotatedBox3(img, x, y, w, l, yaw, color,u_a):
    bev_corners = get_corners(x, y, w, l, yaw)
    corners_int = bev_corners.reshape(-1, 1, 2).astype(int)
    cv2.polylines(img, [corners_int], True, color, 2)
    cv2.putText(img, 'Hello', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    ...
    font_scale = 1
    font_color = (255, 255, 255)  # 文字颜色，这里是白色
    thickness = 2  # 文字线条粗细
    text_size = cv2.getTextSize(text, font, font_scale, thickness)[0]
    org = ((img.shape[1] - text_size[0]) // 2, (img.shape[0] + text_size[1]) // 2)
    cv2.putText(overlay, text, org, font, font_scale, font_color, thickness)
    img = cv2.addWeighted(img, 1, overlay, 1, 0)
    # reversed_text = text[::-1]

def drawRotatedBox4(img, x, y, w, l, yaw, color, u_a):
    bev_corners = get_corners(x, y, w, l, yaw)
    corners_int = bev_corners.reshape(-1, 1, 2).astype(int)
    cv2.polylines(img, [corners_int], True, color, 2)
    ...
    # text = text[::-1]
    # reversed_text = text[::-1]
    cv2.putText(img, text, (corners_int[-4, 0].astype(np.int32), corners_int[0, 1].astype(np.int32) - 10),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
