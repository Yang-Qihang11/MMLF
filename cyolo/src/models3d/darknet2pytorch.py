...
from cyolo.src.models3d.yolo_layer import YoloLayer
from cyolo.src.models3d.darknet_utils import parse_cfg, print_cfg, load_fc, load_conv_bn, load_conv
    ...
    def forward(self, x, targets=None):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                ...
                x, layer_loss,targets_yolo = self.models[ind](x, targets, img_size, self.use_giou_loss)
                loss += layer_loss
                yolo_outputs.append(x)
                targets_22743.append(targets_yolo)
            elif block['type'] == 'cost':
                continue
            else:
                print('unknown type %s' % (block['type']))
        yolo_outputs = to_cpu(torch.cat(yolo_outputs, 1))
        targets_22743 = to_cpu(torch.cat(targets_22743, 1))

        return yolo_outputs if targets is None else (loss, yolo_outputs,targets_22743)
