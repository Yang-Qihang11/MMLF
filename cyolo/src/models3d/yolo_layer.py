...
from cyolo.src.utils3d.torch_utils import to_cpu
from cyolo.src.utils3d.iou_rotated_boxes_utils import iou_pred_vs_target_boxes, iou_rotated_boxes_targets_vs_anchors, \
...
class YoloLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_size=608, use_giou_loss=False):
        ...
        # Get outputs
        #todo 获取输出 针对每一个锚框，共有10个维度输出，即x、y、w、h、im、re、conf、cls（3类）
        #todo 模型最终输出维度为Bx22743x10，其中22743为锚框的个数，即76x76x3 + 38x38x3 + 19 x19x3
        ...
        if targets is None:
            ...
            if self.use_giou_loss:
                ...
                total_loss = loss_cls+loss_conf_noobj+loss_conf_obj
            else:
            ...
            obj_mask1 = obj_mask.unsqueeze(-1)
            noobj_mask1 = noobj_mask.unsqueeze(-1)
            tconf1 = tconf.unsqueeze(-1)
            obj_and_no_mask = torch.cat((obj_mask1,noobj_mask1),dim=4)
            obj_and_no_mask_cls = torch.cat((obj_and_no_mask,tconf1,tcls),dim=4)
            obj_and_no_mask_cls = obj_and_no_mask_cls.view(1,-1,6)
            #
            #
            # #验证非平铺 + 无延伸
            # obj_mask2 = obj_and_no_mask_cls[:,:,:,:,0].squeeze(dim=-1).bool()
            # noobj_mask2 = obj_and_no_mask_cls[:,:,:,:,1].squeeze(dim=-1).bool()
            #
            # tconf =  obj_and_no_mask_cls[:,:,:,:,2].squeeze(dim=-1)
            # tcls =  obj_and_no_mask_cls[:,:,:,:,3:]
            #
            # are_equal11 = torch.equal(obj_mask2, obj_mask)
            # are_equal22 = torch.equal(noobj_mask2, noobj_mask)
            # #
            # #
            # #
            # #
            # loss_conf_obj1 = F.binary_cross_entropy(pred_conf[obj_mask2], tconf[obj_mask2], reduction=self.reduction)
            # loss_conf_noobj1 = F.binary_cross_entropy(pred_conf[noobj_mask2], tconf[noobj_mask2],
            #                                           reduction=self.reduction)
            # loss_cls1 = F.binary_cross_entropy(pred_cls[obj_mask2], tcls[obj_mask2], reduction=self.reduction)
            #
            # #验证平铺  成功√
            # targets_22743=[]
            # targets_22743.append(output)
            # targets_22743 = torch.cat(targets_22743, 1)
            # pred_conf2 = targets_22743[:,:,-4:-3]
            # pred_cls2 = targets_22743[:,:,-3:]
            #
            #
            # pred_conf = pred_conf.view(num_samples, -1, 1)
            # pred_cls = pred_cls.view(num_samples, -1, self.num_classes)
            #
            # are_equal1 = torch.equal(pred_conf2,pred_conf)
            # are_equal2 = torch.equal(pred_cls2,pred_cls)



            # obj_mask3 = obj_and_no_mask_cls[:, :, :, :, 0].squeeze(dim=-1).bool().view(1,-1)
            # noobj_mask3 = obj_and_no_mask_cls[:, :, :, :, 1].squeeze(dim=-1).bool().view(1,-1)
            # tconf2 = obj_and_no_mask_cls[:, :, :, :, 2].squeeze(dim=-1).view(1,-1,1)
            # tcls2 = obj_and_no_mask_cls[:, :, :, :, 3:].view(1,-1,3)
            # obj_mask2 = obj_and_no_mask_cls[ :, :, 0].squeeze(dim=-1).bool().view(1,-1)
            # noobj_mask2 = obj_and_no_mask_cls[ :, :, 1].squeeze(dim=-1).bool().view(1,-1)
            # tconf2 = obj_and_no_mask_cls[ :, :, 2].squeeze(dim=-1).view(1,-1,1)
            # tcls2 = obj_and_no_mask_cls[:, :, 3:].view(1,-1,3)
            #
            #
            # loss_conf_obj2= F.binary_cross_entropy(pred_conf2[obj_mask2], tconf2[obj_mask2], reduction=self.reduction)
            # loss_conf_noobj2 = F.binary_cross_entropy(pred_conf2[noobj_mask2], tconf2[noobj_mask2],
            #                                           reduction=self.reduction)
            # loss_cls2 = F.binary_cross_entropy(pred_cls2[obj_mask2], tcls2[obj_mask2], reduction=self.reduction)
            return output, total_loss, obj_and_no_mask_cls
