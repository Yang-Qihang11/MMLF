...
import cyolo.src.data_process.kitti_bev_utils as bev_utils
...
def post_processing_v2(prediction, conf_thresh=0.95, nms_thresh=0.4):
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        while detections.size(0):
            ...
            label_match = detections[0, -4] == detections[:, -4]
    ...
    return output,cls_fea



def post_processing_v3(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
        Removes detections with lower object confidence score than 'conf_thres' and performs
        Non-Maximum Suppression to further filter detections.
        Returns detections with shape:
...
#todo 这个为修改过后的nms
def nms(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
        Removes detections with lower object confidence score than 'conf_thres' and performs
        Non-Maximum Suppression to further filter detections.
        Returns detections with shape:
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        # Sort by it
        image_pred = image_pred[(-score).argsort()]
        obj_score = image_pred[:, 6:7]
        class_confs, class_preds = image_pred[:, 7:].max(dim=1, keepdim=True)
        class_confs = obj_score
        detections = torch.cat((image_pred[:, :7].float(), class_confs.float(), class_preds.float()), dim=1)
        cls_fea = image_pred[:,-3:].float()
        # Perform non-maximum suppression
        keep_boxes = []
...
#todo 带u的nms
def nms2(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
        Removes detections with lower object confidence score than 'conf_thres' and performs
        Non-Maximum Suppression to further filter detections.
        Returns detections with shape:
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        image_pred = image_pred[(-score).argsort()]
        obj_score = image_pred[:, 6:7]
        class_confs, class_preds = image_pred[:, 7:10].max(dim=1, keepdim=True)
        class_confs = obj_score

        #todo 最后一位加入u
        detections = torch.cat((image_pred[:, :7].float(), class_confs.float(), class_preds.float(),image_pred[:,-1:]), dim=1)
        # cls_fea = image_pred[:,-3:].float()
        # Perform non-maximum suppression
        keep_boxes = []
        while detections.size(0):
...
#todo 带u x obj score 的nms
def nms3(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
        Removes detections with lower object confidence score than 'conf_thres' and performs
        Non-Maximum Suppression to further filter detections.
        Returns detections with shape:
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        #todo u/confs
        # image_pred[:, -1:] = torch.div(image_pred[:,-1:], obj_score)
...
#todo 带u x obj score 的nms  过滤uncer过大的值 car 0.12 per 0.244 cyc 0.
def nms4(prediction, conf_thresh=0.95, nms_thresh=0.4):
    """
        Removes detections with lower object confidence score than 'conf_thres' and performs
        Non-Maximum Suppression to further filter detections.
        Returns detections with shape:
