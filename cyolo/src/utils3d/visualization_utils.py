...
def show_image_with_boxes(img, objects, calib, show3d=False):
    ...
    if show3d:
        cv2.imshow("img", img2)
    return img2
def show_image_with_boxes2(img, objects, calib,u_a, show3d=False):
    ''' Show image with 2D bounding boxes '''
    i=0
    img2 = np.copy(img)  # for 3d bbox
    for obj in objects:
        ...
        if box3d_pts_2d is not None:
            img2 = kitti_data_utils.draw_projected_box3d2(img2, box3d_pts_2d, cnf.colors[obj.cls_id],u_a=u_a[i])
        i = i + 1
    if show3d:
        cv2.imshow("img", img2)
    return img2

def show_image_with_boxes3(img, objects, calib, show3d=False):
    ''' Show image with 2D bounding boxes '''
    i = 0
    img2 = np.copy(img)  # for 3d bbox
    for obj in objects:
        ...
        if box3d_pts_2d is not None:
            img2 = kitti_data_utils.draw_projected_box3d(img2, box3d_pts_2d, [0, 0, 255])
            i = i + 1
    if show3d:
        cv2.imshow("img", img2)
    return img2
...
def predictions_to_kitti_format(img_detections, calib, img_shape_2d, img_size, RGB_Map=None):
    ...
    return objects_new
def predictions_to_kitti_format2(img_detections, calib, img_shape_2d, img_size, RGB_Map=None):
    predictions = []
    for detections in img_detections:
        if detections is None:
            continue
        # Rescale boxes to original image
        # Rescale boxes to original image
        for x, y, w, l, im, re, *_, cls_pred,u_a in detections:
            predictions.append([cls_pred, x / img_size, y / img_size, w / img_size, l / img_size, im, re])
